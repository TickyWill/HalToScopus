...
def _extract_hal_dois(institute, corpus_year, working_folder_path, hal_files_tup, scopus_dois_set):
    """Sets DOIs list from HAL not in DOIs list extracted from scopus database."""
    ...
    message += f"saved as '{dois_file_alias}.xlsx' in: \n{working_folder_path}"
    
    return message, hal_not_scopus_doi_list


def consolidate_scopus(institute, corpus_year, working_folder_path, files_tup):   
    """Complements the scopus extraction with information on publications of which DOIs are found in HAL extraction.
    
    Args:
        institute (str): Institute name.
        corpus_year (str): 4 digits year of the corpus.
        working_folder_path (path): Full path to working folder.
        files_tup (tup): (name of the initial Scopus extraction file, name of the updated Scopus file, 
        name for the HAL extraction file, name for the file where DOIs list not present in the initial 
        Scopus extraction is saved, name of the file where publications information of DOIs not found 
        in the Scopus database is saved, name of the file where publications information added to the 
        initial Scopus extraction is saved).
    Returns:
        (tup): (message for exe log, authentication status on Scopus database, 
        Scopus extraction update status).
    """
    # Setting already extracted DOIs list from scopus database
    init_scopus_file = files_tup[0]
    scopus_dois_set, scopus_init_df = _get_scopus_dois(init_scopus_file, working_folder_path)

    # Building DOIs list from HAL not in DOIs list extracted from scopus database
    hal_files_tup = (files_tup[2], files_tup[3])
    message, hal_not_scopus_doi_list = _extract_hal_dois(institute, corpus_year, working_folder_path,
